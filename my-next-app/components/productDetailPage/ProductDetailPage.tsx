...
export default function ProductDetailClient({ product }: Props) {
  ...
  // ✅ Properly handle variant selection
...
/>
          ...
          <div className="space-x-4 flex">
            <div className="text-2xl font-bold text-gray-800">₹{product.sellingPrice}</div>
            {discount > 0 && (
              <div className="text-green-600 font-semibold">{discount}% Off</div>
            )}
            {basePrice && (
              <div className="text-sm text-gray-500">
                MRP <s>₹{product.basePrice}</s> Inclusive of all taxes
              </div>
            )}
          </div>
