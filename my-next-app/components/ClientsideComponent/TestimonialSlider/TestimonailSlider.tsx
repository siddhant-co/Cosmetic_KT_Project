...
export default function TestimonialSlider({ testimonials }: Props) {
  ...
  return (
    <div className="px-4 lg:px-1 overflow-visible">
      <Swiper
        modules={[Autoplay, Navigation]}
        spaceBetween={30}
        slidesPerView={1}
      ...
      >
        {testimonials.map((t, index) => (
          <SwiperSlide key={t.id} className="overflow-visible">
            <div className="p-2">
              <div
                className={`transition-transform duration-300 ease-in-out bg-white rounded-xl shadow-md p-5 h-[250px] max-w-md mx-auto ${
              ...
              >
                ...
                <p className="text-gray-700 text-sm line-clamp-6">{t.description}</p>
              </div>
            </div>
          </SwiperSlide>
        ))}
